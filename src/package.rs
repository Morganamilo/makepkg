...
impl Makepkg {
    ...
    fn make_archive(
        ...
        if srcpkg {
            ...
            pkgdir = dirs.pkgdir.join(&pkg.pkgname);
    ...
    fn generate_buildinfo(
        ...
        let binfo = dirs.pkgdir.join(&pkg.pkgname).join(".BUILDINFO");
        let mut file = File::options();
    ...
    fn generate_pkginfo(
        ...
        let pkgdir = dirs.pkgdir.join(&pkg.pkgname).join(".PKGINFO");
        let mut file = File::options();
        file.write(true).create(true).truncate(true);
        let mut file = open(
            &file,
    ...
    fn package_size(&self, dirs: &PkgbuildDirs, pkg: &Package) -> Result<u64> {
        let path = dirs.pkgdir.join(&pkg.pkgname);
        let mut size = 0;
